        ...
        cursor.execute(
            """INSERT INTO lesson_plans 
               (id, topic, grade_level, number_of_classes, minutes_per_class, 
                language, original_plan, current_plan, model, offline)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
...
async def get_lesson_plan_history():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, topic, grade_level, number_of_classes, minutes_per_class, model, offline, created_at FROM lesson_plans ORDER BY created_at DESC"
        )
